...
- 커밋 메시지에 `Closes #N`, `Fixes #N`, `Resolves #N`을 포함하면 해당 커밋이 default branch에 push될 때 이슈가 자동으로 close됨
- 여러 이슈 동시 close 가능: `Closes #1, Closes #2, Closes #3`
- 이슈에서 해당 커밋이 자동 참조되어 "어떤 작업으로 이 이슈가 해결되었는지" 추적 가능
- PR 본문에도 동일 키워드 사용 가능 (PR merge 시 이슈 close)
